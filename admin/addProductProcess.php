...
if (empty($title)) {
    ...
    (`name`,`stitle`,`qty`,`price`,`discount`,
    `dprice`,`discription`,`date`,
    `delivery_fee`,`display_id`,
    `brand_id`,`cpu_id`,`ram_id`,
    `gpu_id`,`storage_id`,`produt_status_id`,
    `model_id`,`category_id`,`size_id`,
    `warranty_id`,`bestsell_id`,`flashsell_id`)
     VALUES 
    ('".$title."','".$stitle."','".$qty."',
    '".$price."','".$discount."','".$dprice."',
    '".$desc."','".$date."','".$delivery."',
    '".$display."',
    '".$brand."','".$cpu."','".$ram."',
    '".$gpu."','".$storage."','".$status."',
    ...
    if($length <= 3 && $length > 0){
        $allowed_img_extentions = array ("image/jpg","image/jpeg",
        ...
        for($x = 0; $x < $length;$x++){
            if(isset($_FILES["image".$x])){
                ...
                if(in_array($file_extention,$allowed_img_extentions)){
                    ...
                    $file_name = "resource/laptop/".$title."".$x.""
                    .uniqid().$new_img_extention;
