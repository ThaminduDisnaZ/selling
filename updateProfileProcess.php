...
if (isset($_SESSION["u"])) {
    ...
    if (isset($_FILES["img"])) {
        $img = $_FILES["img"];
        $allowed_image_extentions = array(
        ...
        if (!in_array($file_ex, $allowed_image_extentions)) {
            ...
            $file_name = "resources/user-profile-images/" . $_SESSION["u"]["fname"] . "_"
                . uniqid() . $new_file_extention;
            move_uploaded_file($img["tmp_name"], $file_name);
            $image_rs = Database::search("SELECT* FROM `user_img` 
            WHERE `user_id`='" . $uid . "'");
            $image_num = $image_rs->num_rows;
            if ($image_num == 1) {
                Database::iud("UPDATE`user_img` SET `path`='" . $file_name . "'
                WHERE `user_id`='" . $uid . "'");
            } else {
                Database::iud("INSERT INTO`user_img`  (`path`,`user_id`)
                 VALUES ('" . $file_name . "','" . $uid . "')");
            }
        }
    }
